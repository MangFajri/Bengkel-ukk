                ...
                <div class="card-body">
                    @if($myVehicles->isEmpty())
                        <div class="text-center py-4">
                            ...
                            <a href="{{ route('customer.vehicles.create') }}" class="btn btn-sm btn-primary">Daftarkan Sekarang</a>
                        </div>
                    @else
                        <ul class="list-group list-group-flush">
                            @foreach($myVehicles as $car)
                            <li class="list-group-item d-flex justify-content-between align-items-center px-0" style="background-color: transparent; border-bottom: 1px solid #334155;">
                                <div class="d-flex align-items-center">
                                    <div class="mr-3 bg-primary rounded-circle d-flex align-items-center justify-content-center text-white" style="width: 40px; height: 40px;">
                                ...
                                <tbody>
                                    ...
                                    <tr style="border-bottom: 1px solid #334155;">
                                        ...
                                        <td class="py-3 text-right">
                                            @php
                                                $status = strtolower($trx->serviceStatus->name);
                                                $badge = 'secondary';
                                                if(str_contains($status, 'selesai')) $badge = 'success';
                                                elseif(str_contains($status, 'pending')) $badge = 'warning';
                                                elseif(str_contains($status, 'kerja')) $badge = 'info';
                                            @endphp
                                            <span class="badge badge-{{ $badge }}">{{ $trx->serviceStatus->name }}</span>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
